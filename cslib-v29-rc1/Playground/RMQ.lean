...
private theorem buildTable_invariant (a : Array ℕ) (m : ℕ)
  ...
  | zero =>
    have hi0 : i = 0 := by omega
    subst hi0
    grind [rangeMin_one, buildTable]
  | succ m ih =>
    have hm' : 2 ^ m ≤ a.size :=
      le_trans (Nat.pow_le_pow_right (by omega) (Nat.le_succ m)) hm
    by_cases him : i ≤ m
    · -- Old row preserved by push
      have hlt : i < (buildTable a m).size := by grind [buildTable_size]
      have heq : (buildTable a (m + 1))[i]'(by grind [buildTable_size]) =
...
theorem table_invariant (a : Array ℕ) (ha : 0 < a.size) (i j : ℕ)
  ...
  have hi' : i ≤ Nat.log 2 a.size := by grind [buildTable_eq_make, buildTable_size]
  have hj' : j + 2 ^ i ≤ a.size := by
    have : 2 ^ i ≤ a.size := le_trans (Nat.pow_le_pow_right (by grind) hi') hm
    grind [buildTable_eq_make, buildTable_row_size, buildTable_size]
  simp only [buildTable_eq_make a ha]
  exact buildTable_invariant a _ hm i j hi' hj'
...
: (SparseTable.make vals).query l r = rmqNaive vals l r := by
  ...
  have hr1_lt : r + 1 - 2 ^ k < ((SparseTable.make vals).table[k]'hk_lt).size := by
    rw [hrow_sz]; omega
  have inv1 : ↑(((SparseTable.make vals).table[k]'hk_lt)[l]'hl_lt) =
    rangeMin vals.toList l (2 ^ k) :=
    table_invariant vals ha k l hk_lt hl_lt
  have inv2 : ↑(((SparseTable.make vals).table[k]'hk_lt)[r + 1 - 2 ^ k]'hr1_lt) =
    rangeMin vals.toList (r + 1 - 2 ^ k) (2 ^ k) :=
    table_invariant vals ha k (r + 1 - 2 ^ k) hk_lt hr1_lt
  have haddr : l + len - 2 ^ k = r + 1 - 2 ^ k := by grind
  rw [rmqNaive_eq_rangeMin]
  rw [show r - l + 1 = len from rfl]
  rw [← rangeMin_overlap vals.toList l len k hpow_le hpow_lt, haddr]
  rw [← inv1, ← inv2, ← WithTop.coe_min]
  congr 1
  simp only [SparseTable.query]
  simp only [show Nat.log 2 (r - l + 1) = k from rfl]
  grind
