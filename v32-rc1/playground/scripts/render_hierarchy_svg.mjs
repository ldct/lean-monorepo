// Render scripts/hierarchy.dot to scripts/hierarchy_rendered.svg with the
...
// Legend chips (L1..) get no data attributes.
//
// Setup:  npm i @viz-js/viz   (anywhere; point NODE_PATH at its node_modules)
// Run:    NODE_PATH=<dir>/node_modules node scripts/render_hierarchy_svg.mjs
...
const dot = readFileSync(join(here, 'hierarchy.dot'), 'utf8');
...
svg = svg.replace(
  ...
  (m, open, mid, title, close) => {
    ...
    if (/^L\d+$/.test(name)) return m;
    nodeCount++;
    const a = escAttr(name);
    return `${open} data-class-name="${a}" tabindex="0" role="img" aria-label="${a}"${mid}${title}${close}`;
  });

writeFileSync(join(here, 'hierarchy_rendered.svg'), svg);
console.log(`wrote scripts/hierarchy_rendered.svg (${nodeCount} data-class-name nodes, ${svg.length} bytes)`);
