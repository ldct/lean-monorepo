...
Input:  scripts/hierarchy_input.json  (nodes, edges, owners, aliases, roster)
...
/-- Real Lean class + optional fixed extra argument position for diagram aliases. -/
def realizeClass (diagName : String) : Name × Option (Nat × Name) :=
  match diagName with
  | "NPow"  => (`Pow,  some (1, `Nat))
  | "ZPow"  => (`Pow,  some (1, `Int))
  | "NSMul" => (`SMul, some (0, `Nat))
  | "ZSMul" => (`SMul, some (0, `Int))
  | _       => (diagName.toName, none)
...
def matchesClass (real : Name) (extra : Option (Nat × Name)) (e : Expr) : Bool :=
  let fn := e.getAppFn
  match fn with
  | .const n _ =>
    n == real &&
      (match extra with
       ...
       | some (i, argConst) =>
         ...
         i < args.size && args[i]!.isConstOf argConst)
  | _ => false
...
#eval show Elab.Command.CommandElabM Unit from do
  ...
  Elab.Command.liftTermElabM do
    ...
    for e in spec.edges do
      let (realS, extraS) := realizeClass e.s
      let (realT, extraT) := realizeClass e.t
      ...
      if let some (_, d) := manualEdgeDecl.find? (·.1 == s!"{e.s}->{e.t}") then
        ...
        for (n, _, ty) in instInfos do
          ...
          unless matchesClass realT extraT body do continue
          let instBinders := binders.filter (fun (bi, _) => bi == .instImplicit)
          unless instBinders.any (fun (_, t) => matchesClass realS extraS t) do continue
    ...
    let realToDiag : Std.HashMap Name String := Id.run do
      let mut m := {}
      for n in spec.nodes do
        let (real, extra) := realizeClass n
        if extra.isNone then m := m.insert real n
      return m
