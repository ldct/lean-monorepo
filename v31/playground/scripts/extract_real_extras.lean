/-
Extra metadata for site/real_instances.html (classes satisfied by ℝ).
Adapted from scripts/extract_hierarchy_extras.lean; differences:
  * input scripts/real_input.json, output site/real_extras.json
  * no diagram aliases: every node is a real (possibly namespaced) Lean class
  * edge backing declarations: dashed edges carry the exact backing `instance`
    name discovered by scripts/discover_real_classes.lean (edge field `decl`);
    solid edges resolve the parent projection via `getStructureParentInfo`
    (correct for namespaced classes, unlike the `S.toT` string heuristic);
    instance-table search kept as a fallback.
Same output schema: per-field isProp/default/autoParam, per-class
isProp/instance-matrix/constructors, per-edge kind/decl/prio/doc/type.
Run: lake env lean scripts/extract_real_extras.lean
-/
import Mathlib
...
/-- Does `e` (an application, possibly with loose bvars) have head constant `real`? -/
  ...
  Elab.Command.liftTermElabM do
    ...
    let classIsProp : Std.HashMap String Bool := Id.run do
      ...
      for n in spec.nodes do
        let isP := match env.find? n.toName with
          | some ci =>
    ...
    for cls in spec.nodes do
      ...
      for h : i in [0:spec.roster.size] do
        ...
        let ok ← withCurrHeartbeats <| withOptions (fun o => o.set `maxHeartbeats (200000 : Nat)) do
          try
            match Parser.runParserCategory env `term s!"{cls} ({ty})" with
            | .error _ => pure false
            | .ok stx =>
    ...
    -- ================= edge declarations =================
    -- pre-scan the instance table once (fallback resolution only)
    ...
    for e in spec.edges do
      ...
      -- 1. exact backing instance recorded at discovery time (dashed edges)
      if let some d := e.decl then
        if env.contains d.toName then
          decl := some d.toName
      -- 2. parent projection (solid edges), via structure-parent info
      if decl.isNone then
        for p in getStructureParentInfo env realS do
          if p.structName == realT then
            decl := some p.projFn
      -- 3. fallback: instance-table search (result matches T, some inst-binder matches S)
      if decl.isNone then
    ...
    IO.FS.writeFile "site/real_extras.json" out.compress
    IO.println "wrote site/real_extras.json"
