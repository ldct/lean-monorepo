...
example {n m : ℤ} : (MultiplesOf n) + (MultiplesOf m) = (MultiplesOf (n.gcd m)) := by
  ...
  · -- TODO(v31 port): the sup was previously handled by definitional unfolding of
    -- `iInter`; here we give a direct proof via Bezout's identity.
    intro x hx
    -- Since `x ∈ MultiplesOf (n.gcd m)`, write `x = ↑(n.gcd m) * k`.
    obtain ⟨k, hk⟩ := hx
    -- By Bezout's identity, `↑(n.gcd m) = a * n + b * m`, hence it lies in the sum.
    have hgcd_mem : (↑(n.gcd m) : ℤ) ∈ MultiplesOf n + MultiplesOf m := by
      obtain ⟨a, b, hab⟩ : ∃ a b : ℤ, a * n + b * m = Int.gcd n m :=
        ⟨Int.gcdA n m, Int.gcdB n m, by rw [Int.gcd_eq_gcd_ab n m]; ring⟩
      rw [← hab]
      exact Submodule.add_mem _
        (Submodule.mem_sup_left ⟨a, by ring⟩)
        (Submodule.mem_sup_right ⟨b, by ring⟩)
    -- Then `x = ↑(n.gcd m) * k = k • ↑(n.gcd m)` is also in the sum.
    rw [← hk, mul_comm, ← smul_eq_mul]
    exact (MultiplesOf n + MultiplesOf m).smul_mem k hgcd_mem
