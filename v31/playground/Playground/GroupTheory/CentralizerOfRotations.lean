...
theorem s_not_in_CA (hn : 2 < n) : (sr 0) ∉ (C_A n) := by
  intro hs
  specialize hs (r 1)
  simp only [SetLike.mem_coe, r_mul_sr, zero_sub, sr_mul_r, zero_add, sr.injEq] at hs
...
example (i : ZMod n) (hn : 2 < n) : (sr i ∉ (C_A n)) := by
  ...
  simp only [sr_mul_r, add_neg_cancel] at mul
...
theorem CA_le_A (hn : 2 < n) : (C_A n) ≤ (Rot n) := by
  ...
  simp only [sr_mul_r, add_neg_cancel] at prod_in_CA
  apply s_not_in_CA
  · exact hn
  · exact prod_in_CA
