  ...
  iseqv := {
    ...
    trans := by
      ...
      have h : x.numerator * z.denominator * y.denominator= z.numerator * x.denominator * y.denominator := by
        grind
      have hyz : (y.denominator : ℤ) ≠ 0 := Nat.cast_ne_zero.mpr y.nonzero
...
@[simp]
theorem PreRat.eq (a c : ℤ) (b d : ℕ) (hb : b ≠ 0) (hd : d ≠ 0) :
...
abbrev Rat.formalDiv (a : ℤ) (b : ℕ) : Rat :=
...
@[grind]
theorem Rat.eq (a c : ℤ) {b d : ℕ} (hb : b ≠ 0) (hd : d ≠ 0) : a // b = c // d ↔ a * d = c * b := by
...
instance Rat.add_inst : Add Rat where
  add := Quotient.lift₂ (fun ⟨ a, b, h1 ⟩ ⟨ c, d, h2 ⟩ ↦ (a*d+b*c) // (b*d)) (by
    ...
    have hbd : b * d ≠ 0 := Nat.mul_ne_zero h1 h2
    have hbd' : b' * d' ≠ 0 := Nat.mul_ne_zero h1' h2'
    ...
    push_cast
    linear_combination (↑d : ℤ) * ↑d' * h3 + (↑b : ℤ) * ↑b' * h4
  )

/-- Definition 4.2.2 (Addition of rationals) -/
theorem Rat.add_eq (a c : ℤ) {b d : ℕ} (hb : b ≠ 0) (hd : d ≠ 0) :
    (a // b) + (c // d) = (a*d + b*c) // (b*d) := by
  simp only [Rat.formalDiv, dif_pos hb, dif_pos hd]; rfl
...
instance Rat.mul_inst : Mul Rat where
  mul := Quotient.lift₂ (fun ⟨ a, b, h1 ⟩ ⟨ c, d, h2 ⟩ ↦ (a*c) // (b*d)) (by
    ...
    push_cast
    linear_combination c * (↑d' : ℤ) * h3 + a' * (↑b : ℤ) * h4
  )

/-- Definition 4.2.2 (Multiplication of rationals) -/
theorem Rat.mul_eq (a c : ℤ) {b d : ℕ} (hb : b ≠ 0) (hd : d ≠ 0) :
    (a // b) * (c // d) = (a*c) // (b*d) := by
  simp only [Rat.formalDiv, dif_pos hb, dif_pos hd]; rfl
...
theorem Rat.neg_eq (a : ℤ) {b : ℕ} (hb : b ≠ 0) : - (a // b) = (-a) // b := by
  show (Quotient.lift _ _ _ : Rat) = _
  rw [Rat.formalDiv, dif_pos hb]; rfl
...
lemma mul_nz {a b : ℕ} (ha : a ≠ 0) (hb : b ≠ 0) :
  (a * b ≠ 0) := by
  by_contra h
  rw [mul_eq_zero] at h
  tauto
...
AddGroup.ofLeftAxioms (by
  ...
  have hbd : b*d ≠ 0 := mul_nz hb hd
  have hdf : d*f ≠ 0 := mul_nz hd hf
  have hbdf : b*d*f ≠ 0 := mul_nz hbd hf
...
def my_inv (a : PreRat) : Rat :=
  if a.numerator < 0 then -a.denominator // a.numerator.natAbs
  else a.denominator // a.numerator.natAbs

lemma my_inv_num_neg (a : PreRat) (ha : a.numerator < 0) : my_inv a = -a.denominator // a.numerator.natAbs := by
  unfold my_inv
  simp [ha]
