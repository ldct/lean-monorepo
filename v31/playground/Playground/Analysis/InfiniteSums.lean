...
: Monotone (partialSums a) := by
  -- In a general preorder, proving monotonicity requires checking f(x) ≤ f(y) for all pairs
  -- {(x, y) | x ≤ y}, but since ℕ is discrete, we can just check adjacent pairs {(x, x+1) | x ∈ ℕ}
  apply monotone_nat_of_le_succ
  intro x
  unfold partialSums
  -- f(x) ≤ f(x+1) becomes ∑ range(x) ai ≤ ∑ range(x+1) ai, and we can rewrite the RHS as
  -- ∑ range(x) ai + (x-th term)
  rw [Finset.sum_range_succ_comm]
