/-
https://github.com/leanprover-community/mathlib4/pull/22039
-/
import Mathlib
import Lean.Expr
import Qq
...
-- mkNatLitQ and mkIntLitQ are now provided by mathlib
...
simproc Icc_ofNat_ofNat (Icc _ _) := .ofQ fun u α e ↦ do
  match u, α, e with
...
* Make sure to exclude `Finset.insert_eq_of_mem` from your simp call when using this simproc. This
  avoids a quadratic time performance hit. -/
simproc Ico_ofNat_ofNat (Ico _ _) := .ofQ fun u α e ↦ do
  match u, α, e with
...
* Make sure to exclude `Finset.insert_eq_of_mem` from your simp call when using this simproc. This
  avoids a quadratic time performance hit. -/
simproc Ioc_ofNat_ofNat (Ioc _ _) := .ofQ fun u α e ↦ do
  match u, α, e with
...
* Make sure to exclude `Finset.insert_eq_of_mem` from your simp call when using this simproc. This
  avoids a quadratic time performance hit. -/
simproc Ioo_ofNat_ofNat (Ioo _ _) := .ofQ fun u α e ↦ do
  match u, α, e with
