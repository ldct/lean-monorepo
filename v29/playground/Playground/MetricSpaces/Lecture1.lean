...
def IsOpen {X : Type} (M : IMetricSpace X) (A : Set X) : Prop :=
...
@[fun_prop]
lemma UnitIntervalC.intervalIntegrable_zero_one {k : ℕ} (u : UnitIntervalC k) :
    IntervalIntegrable u.toFun MeasureTheory.volume 0 1 :=
  u.contDiff.continuousOn.intervalIntegrable_of_Icc (by norm_num)
...
lemma integral01'_intervalIntegrable (f g : UnitIntervalC 0) :
    IntervalIntegrable (fun x => |(|f.toFun x - g.toFun x|)|) MeasureTheory.volume 0 1 := by
  fun_prop
...
noncomputable def L1Metric : IMetricSpace (UnitIntervalC 0) where
  ...
  d_zero_iff_eq := by
    ...
    · intro h
      ...
      by_cases hx : x ∈ unitInterval
      · by_contra hne
        have hpos : 0 < |(|f.toFun x - g.toFun x|)| := by
          rw [abs_abs]; exact abs_pos.mpr (sub_ne_zero.mpr hne)
        clear hne
        have integ_pos : 0 < integral01' f g := by
          unfold integral01' integral01
          refine intervalIntegral.integral_pos (by norm_num) ?_ ?_ ⟨x, hx, hpos⟩
          · fun_prop
          · intro y _; positivity
        linarith
      · rw [f.vanishesOutside x hx, g.vanishesOutside x hx]
    · rintro rfl
      simp [integral01', integral01]
