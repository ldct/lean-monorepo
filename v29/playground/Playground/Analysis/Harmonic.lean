...
theorem e1 (k : ℕ) :
    (∑ i ∈ Finset.Ico (2 ^ k) (2 ^ (k + 1)), invNats i) ≥
    (∑ _ ∈ Finset.Ico (2 ^ k) (2 ^ (k + 1)), invNats (2^(k+1)-1)) := by
  gcongr with i hi
  unfold invNats
...
theorem e2 (k : ℕ) : (∑ _ ∈ Finset.Ico (2 ^ k) (2 ^ (k + 1)), invNats (2^(k+1)-1)) = (1/2) := by
  have : 2^(k + 1) - 2^k = 2^k := by
    ...
    omega
  unfold invNats
  simp only [Order.lt_two_iff, zero_le, pow_succ_pos, Nat.cast_pred, Nat.cast_pow, Nat.cast_ofNat,
    sub_add_cancel, one_div, Rat.cast_inv, Rat.cast_pow, Rat.cast_ofNat, Finset.sum_const,
...
theorem s_unbounded_formula (k : ℕ) : s (2^k) ≥ 1 + (k:ℝ)/2 := by
  induction k with
  | zero =>
    simp only [pow_zero, CharP.cast_eq_zero, zero_div, add_zero, ge_iff_le]
  ...
  | succ k IH =>
    ...
    rw [← Finset.sum_Ico_consecutive invNats (by positivity)
      (show 2^k ≤ 2^(k+1) by gcongr <;> omega)]
    have : (∑ i ∈ Finset.Ico (2 ^ k) (2 ^ (k + 1)), invNats i) ≥ 1/2 := by
      calc
        (∑ i ∈ Finset.Ico (2 ^ k) (2 ^ (k + 1)), invNats i) ≥
          (∑ _ ∈ Finset.Ico (2 ^ k) (2 ^ (k + 1)), invNats (2^(k+1)-1)) := e1 k
        _ = 1/2 := e2 k
    have t := calc
      (∑ i ∈ Finset.Ico 0 (2 ^ k), invNats i +
          ∑ i ∈ Finset.Ico (2 ^ k) (2 ^ (k + 1)), invNats i) =
          (∑ i ∈ Finset.range (2 ^ k), invNats i +
          ∑ i ∈ Finset.Ico (2 ^ k) (2 ^ (k + 1)), invNats i) := by
        congr
        rw [congrFun Finset.range_eq_Ico]
      (∑ i ∈ Finset.range (2 ^ k), invNats i +
          ∑ i ∈ Finset.Ico (2 ^ k) (2 ^ (k + 1)), invNats i) ≥
          (1 + k/2) + ∑ i ∈ Finset.Ico (2 ^ k) (2 ^ (k + 1)), invNats i := by
        gcongr
      _ ≥ 1 + k/2 + 1/2 := by
        gcongr
      _ = 1 + (k+1)/2 := by ring
