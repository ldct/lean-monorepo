...
private def M4 : Matrix (Fin 4) (Fin 4) ℤ :=
  !![ 2,  1,  0,  0;
      ...
      0,  1,  2,  1;
      0,  0,  1,  2]
example : M4.det = 5 := by simp only [intDetSimproc]
...
theorem E₈_det : E₈.det = 1 := by simp only [intDetSimproc]
...
example : SwapNeeded2.det = -1 := by simp only [intDetSimproc]

-- 3×3 with first pivot zero.
private def SwapNeeded3 : Matrix (Fin 3) (Fin 3) ℤ :=
  !![0, 1, 2; 3, 4, 5; 6, 7, 9]
example : SwapNeeded3.det = -3 := by simp only [intDetSimproc]
...
theorem E₈_perm_det : E₈_perm.det = 1 := by simp only [intDetSimproc]
...
example : Sing.det = 0 := by
  simp only [Sing, intDetSimproc]; decide
