...
theorem isCyclic_of_card_pq {p q : ℕ} [Fact p.Prime] [Fact q.Prime] (hqp : q < p) (hdiv : ¬ q ∣ p - 1) (G : Type*) [Group G] [Fintype G] (hG : Fintype.card G = p * q) : IsCyclic G := by
  ...
  obtain ⟨P, hP⟩ : ∃ P : Subgroup G, Fintype.card P = p ∧ P.Normal := by
    ...
    obtain ⟨P, hP⟩ : ∃ P : Sylow p G, ∀ Q : Sylow p G, Q = P := by
      ...
      have h_sylow_p_count : (Nat.card (Sylow p G)) ≡ 1 [MOD p] ∧ (Nat.card (Sylow p G)) ∣ q := by
        have h_sylow_p : Nat.card (Sylow p G) ≡ 1 [MOD p] := card_sylow_modEq_one p G
        have h_sylow_p_div : Nat.card (Sylow p G) ∣ Fintype.card G := by
          -- By Sylow's theorems, the number of Sylow $p$-subgroups divides the group order.
          rw [← Nat.card_eq_fintype_card]
          exact (Sylow.card_dvd_index (Classical.arbitrary (Sylow p G))).trans
      ...
      exact Nat.card_eq_one_iff_exists.mp h_unique;
  ...
  obtain ⟨Q, hQ⟩ : ∃ Q : Subgroup G, Fintype.card Q = q ∧ Q.Normal := by
    ...
    have h_nq : (Nat.card (Sylow q G)) = 1 := by
      ...
      have h_nq : (Nat.card (Sylow q G)) ≡ 1 [MOD q] ∧ (Nat.card (Sylow q G)) ∣ p := by
        have h_sylow_count : (Nat.card (Sylow q G)) ≡ 1 [MOD q] ∧ (Nat.card (Sylow q G)) ∣ Fintype.card G := by
          refine ⟨card_sylow_modEq_one q G, ?_⟩
          rw [← Nat.card_eq_fintype_card]
          exact (Sylow.card_dvd_index (Classical.arbitrary (Sylow q G))).trans
            (Subgroup.index_dvd_card _)
        obtain ⟨left_1, right_1⟩ := h_sylow_count;
        refine ⟨left_1, ?_⟩;
        exact ( Nat.Coprime.dvd_of_dvd_mul_right ( show Nat.Coprime ( Nat.card ( Sylow q G ) ) q from Nat.Coprime.symm <| Nat.Prime.coprime_iff_not_dvd ( Fact.out ) |>.2 fun h => by have := Nat.modEq_zero_iff_dvd.mp ( left_1.symm.trans <| Nat.modEq_zero_iff_dvd.mpr h ) ; simp_all +decide [ Nat.ModEq, Nat.mod_eq_of_lt Fact.out ] ) ( hG ▸ right_1 ) );
    ...
    obtain ⟨Q, hQ⟩ : ∃ Q : Sylow q G, ∀ R : Sylow q G, R = Q := by
      exact Nat.card_eq_one_iff_exists.mp h_nq;
