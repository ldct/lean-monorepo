...
theorem ConvergesThenBounded {f : ℕ → ℝ} (hc : Converges f) : Bounded f := by
  obtain ⟨l, hFTendsToL⟩ := hc
  specialize hFTendsToL 1 (by norm_num)
  obtain ⟨N, hN⟩ := hFTendsToL
...
: TendsTo (fun n ↦ c * a n) (c * A) := by
  intro ε hε
  dsimp
  obtain ⟨B, hB⟩ := ha (ε / |c|) (by positivity)
  use B
  intro n hn
...
theorem tendsTo_mul_constant
  ...
  (ha : TendsTo a A)
: TendsTo (fun n ↦ c * a n) (c * A) := by
  rcases (eq_or_ne c 0) with c_eq_0 | c_ne_0
  · rw [c_eq_0]
    ring_nf
...
: TendsTo (fun n ↦ a n * b n) (A * B) := by
  ...
  obtain ⟨M, hM⟩ := h_b_bounded
  obtain ⟨m_pos, m_bounds⟩ := hM
  have h_exists_n1 := hb (ε/(2*|A|)) (by positivity)
  obtain ⟨n1, hn1⟩ := h_exists_n1
  have h_exists_n2 := ha (ε/(2*M)) (by positivity)
  obtain ⟨n2, hn2⟩ := h_exists_n2
  use max n1 n2
  intro n hn
  calc
    |a n * b n - (A * B)| = |a n * b n - A * b n + (A * b n - A * B)| := by ring_nf
    _ ≤ |a n * b n - A * b n| + |A * b n - A * B| :=
        abs_add_le (a n * b n - A * b n) (A * b n - A * B)
...
: TendsTo (fun n ↦ 1 / b n) (1 / B) := by
  intro ε hε
  obtain ⟨N₁, hN₁⟩ := hb (|B|/2) (by positivity)
  obtain ⟨N₂, hN₂⟩ := hb (ε * |B|^2 / 2) (by positivity)
  use max N₁ N₂
  intro n hn
  dsimp
