/-
Regenerates per-class tooltip metadata for site/normedfield_hierarchy_staging.html:
...
Input:  scripts/hierarchy_input_staging.json  (nodes)
Output: site/tooltip_data_staging.json  (formatted like Python json.dumps(..., ensure_ascii=False))
...
#eval show Elab.Command.CommandElabM Unit from do
  let inputRaw ← IO.FS.readFile "scripts/hierarchy_input_staging.json"
  ...
  Elab.Command.liftTermElabM do
    ...
    IO.FS.writeFile "site/tooltip_data_staging.json" out
    IO.println s!"wrote site/tooltip_data_staging.json ({entries.size} entries)"
