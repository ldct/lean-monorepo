...
This project request had uuid: 04d58e80-e6d8-4c34-81bb-a2ba4500d0ba

The following was proved by Aristotle:

- example {n : ℕ} (hn : 6 ≤ n)
: Nat.card (MulAut (DihedralGroup n)) = n * Nat.totient n
-/
...
lemma dihedralMap_bijective {n : ℕ} (k : (ZMod n)ˣ) (m : ZMod n) : Function.Bijective (dihedralMap (k : ZMod n) m) := by
  ...
  · rintro ⟨ ⟩;
    ...
    · rename_i a;
      exact ⟨ DihedralGroup.sr ( k⁻¹ * ( a - m ) ), by simp +decide [ mul_sub, sub_mul ] ⟩
...
-/
lemma isReflection_of_order_two_not_central {n : ℕ} (hn : 3 ≤ n) (x : DihedralGroup n) (h2 : orderOf x = 2) (hc : x ∉ Subgroup.center (DihedralGroup n)) : ∃ m : ZMod n, x = DihedralGroup.sr m := by
...
-/
def dihedralAut {n : ℕ} (k : (ZMod n)ˣ) (m : ZMod n) : MulAut (DihedralGroup n) :=
  ...
  obtain ⟨m, hm⟩ : ∃ m : ZMod n, f (DihedralGroup.sr 0) = DihedralGroup.sr m := by
    ...
    have h_sr : f (DihedralGroup.sr 0) ∈ {x : DihedralGroup n | ¬x ∈ Subgroup.center (DihedralGroup n)} := by
      ...
      have h_center_preserved : ∀ x : DihedralGroup n, x ∈ Subgroup.center (DihedralGroup n) ↔ f x ∈ Subgroup.center (DihedralGroup n) := by
        simp +decide [ Subgroup.mem_center_iff ];
        intro x; constructor <;> intro h g <;> have := h ( f.symm g ) <;> simp_all +decide [ mul_assoc ] ;
