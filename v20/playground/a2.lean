...
#check Setoid.Partition.orderIso
#check MySetoid

example {G} [Group G] (x y : G) : Subgroup.instMulAction x y = x * y := rfl

variable (G : Type) [Fintype G] (p : Set (Set G))
#synth Fintype p


example {G} (partition : Set (Set G)) [Fintype partition] (h : Setoid.IsPartition partition)
: Nat.card G = ∑ s ∈ partition, Nat.card s := by
  sorry

example {G} [Group G] [Fintype G] (H : Subgroup G) (x : G) : ∃ n, Nat.card G = n * Nat.card H := by
  have := Setoid.isPartition_classes (MySetoid (Subgroup.instMulAction : MulAction H G))
  unfold Setoid.classes at this
  unfold MySetoid at this
  simp at this
  have : Fintype (Set G) := inferInstance
  have : Monoid G := inferInstance
  #check Group G
