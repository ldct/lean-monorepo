...
latex_pp_app_rules (const := sylvester)
  | _, #[n] => do
...
@[simp] theorem sylvester_one : sylvester 1 = 3 := rfl
@[simp] theorem sylvester_two : sylvester 2 = 7 := rfl
@[simp] theorem sylvester_three : sylvester 3 = 43 := rfl
@[simp] theorem sylvester_four : sylvester 4 = 1807 := rfl
@[simp] theorem sylvester_five : sylvester 5 = 3263443 := rfl
...
-- define the sum ∑ 1/(sylvester i)
def ss (n : ℕ) := ∑ i ∈ Finset.range n, (1:ℚ) / sylvester i

example : ss 4 = (1805 : ℚ) / 1806 := by
  unfold ss
  simp [Finset.sum_range_succ]
  norm_num

#check ((HSub.hSub 3 4) : ℚ)
#check (↑(3 - 4) : ℚ)

example
  (j : ℕ)
: ss j = 1 - (1 : ℚ) / (sylvester j - 1) := by
  induction j
  case zero =>
    simp [ss]
    norm_num
